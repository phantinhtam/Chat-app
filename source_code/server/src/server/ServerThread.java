...
public class ServerThread implements Runnable {
    ...
    public ServerThread(int port, ServerMainForm serverMainForm){
        serverMainForm.appendMessage("[Server]: Khởi động máy chủ trong cổng " + port);
        try {
            ...
            serverMainForm.appendMessage("[Server]: Host:" 
                    ...
                    + ",Port:" + port 
                    + ",Backlog:" + backlog);
            serverMainForm.appendMessage("[Server]: Máy chủ đã bắt đầu.!");
        } 
    ...
    public void stop(){
        try {
            server.close();
            keepGoing = false;
            System.out.println("Máy chủ hiện đã đóng.!");
        } catch (IOException iOException) {
