...
public class ClientThread implements Runnable{
    ...
    public ClientThread(Socket socket, ClientMainForm main){
        ...
        try {
            ...
            main.appendMessage("[IOException]: " + iOException.getMessage(), "Lỗi", Color.RED, Color.RED);
        }
    }
        ...
        try {
            while(!Thread.currentThread().isInterrupted()){
                ...
                switch(CMD){
                    ...
                    case "CMD_FILE_XD":  // Format:  CMD_FILE_XD [sender] [receiver] [filename]
                        ...
                        if(confirm == 0){ // client accepted the request, then inform the sender to send the file now
                            ...
                            try {
                                ...
                                String format = "CMD_SEND_FILE_ERROR " + sender + " Khách hàng từ chối yêu cầu của bạn hoặc kết nối bị mất.!";
                                dos.writeUTF(format);
                            } catch (IOException iOException) {
                                System.out.println("[CMD_FILE_XD]: " + iOException.getMessage());
                            }
                    ...
                    default: 
                        main.appendMessage("[CMDException]: Lệnh không xác định " + CMD, "CMDException", Color.RED, Color.RED);
                    break;
                }
            }
        } catch(IOException iOException){
            main.appendMessage(" Kết nối máy chủ bị mất, vui lòng thử lại sau.!", "Lỗi", Color.RED, Color.RED);
